...
def writesubrun(f):
    ...
    ofile.write('onset\tduration\tintensity\ttrial_type\tstim\tverb\tanim\tAP\tCR\tsyntax\tprobe\n')
    for line in fullfile:
        resp = line.split(',')
        cid = resp[0].split('_')[0] + '_' + resp[0].split('_')[1] + '_' + resp[0].split('_')[3] \
              + '_' + resp[0].split('_')[4] + '_' + resp[0].split('_')[5]
        ofile.write(resp[1] + '\t' + resp[2] + '\t1\t' + cid + '\t' + addcondid(cid) + '\n')
        if (resp[-1] == '') or (resp[-1] == 'None') or (resp[-1] == '0') or (resp[-1] == 0):
            ofile.write(resp[3] + '\t' + resp[4] + '\t0\t' + 'probe' + '\t' + addcondid(None) + '\n')
        else:
