...
def addcondid(st):
    ...
    return str(np.where(stype==st.split('_')[0])[0][0]) + ' ' + \
            str(np.where(verbs==st.split('_')[1])[0][0]) + ' ' + \
            str(np.where(anim==st.split('_')[2])[0][0]) + ' ' + \
            str(np.where(ap==st.split('_')[3])[0][0]) + ' ' + \
            str(np.where(cr==st.split('_')[4])[0][0]) + ' ' + \
            str(np.where(syntax==st.split('_')[3]+ '_' + st.split('_')[4])[0][0])
