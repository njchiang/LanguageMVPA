function [smm_rs, smm_ps, n] = searchlight_LMVPASVM(fullBrainVolumes, models, mask, userOptions, localOptions)
...
%             t_pats=zscore(t_pats / localOptions.nSessions);
    ...
    accs=zeros(1, nModelRDMs);
    for m = 1:length(models)
%         accs(1, m)=LooCvSVM(models(m).label(models(m).label>0), t_pats(models(m).label>0,:), [opts]);
          accs(1,m)=CVpermutationSVM(t_pats(models(m).label>0), models(m).label(models(m).label>0), models(m).runs(models(m).label>0), 'opts', opts);
    end
    %deal with complex
%     mMat=[15 20];
%     for iM=1:length(mMat)
%         m=mMat(iM);
%         tAccs=zeros(1,10);
%         for iN=1:10
%             balanceSet=Shuffle(find(models(m).label==2))';
%             acSet=find(models(m).label==1)';
%             thesePats=t_pats([balanceSet([1:8]) acSet],:);
%             theseLabs=models(m).label([balanceSet([1:8]) acSet],:);
%             theseRuns=models(m).runs([balanceSet([1:8]) acSet],:);
% %             tAccs(iN)=LooCvSVM(theseLabs(theseLabs>0), thesePats(theseLabs>0,:), opts);
%             tAccs(iN)=CVpermutationSVM( thesePats(theseLabs>0,:),theseLabs(theseLabs>0),theseRuns(theseLabs>0),'opts', opts);
%         end
%         accs(1,m)=mean(tAccs);
%     end
    ps=nan;
    vec_ps(cMappingVoxI,:) = ps;
    vec_rs(cMappingVoxI,:)=accs;
    %%
