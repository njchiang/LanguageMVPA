function [smm_rs, smm_ps, n] = searchlight_LMVPA_across_SVM(fullBrainVolumes, models, mask, userOptions, localOptions)
...
parfor cMappingVoxI=1:nVox_mappingMask_request
    ...
    %%%%%%%
    opts1=['-q -t 0']; % linear: default
    opts2=['-q -t 2']; % RBF: default
    %     tic
    %     for m=1:size(models,1)
    %         [rs] = permutationSVM(t_pats, models, runs);
    % randomly grab half of act/pas
    
    lStruct=libsvmtrain(models(1,:)', l_pats, opts1);
    pStruct=libsvmtrain(models(1,:)', p_pats, opts1);
    [~, l2pAnim, ~]=libsvmpredict(models(1,:)', p_pats, lStruct);
    [~, p2lAnim, ~]=libsvmpredict(models(1,:)', l_pats, pStruct);
    l2lAnim=libsvmtrain(models(1,:)', l_pats, [opts1 ' -v 4']);
    p2pAnim=libsvmtrain(models(1,:)', p_pats, [opts1 ' -v 4']);

    lStruct=libsvmtrain(models(2,:)', l_pats, opts1);
    pStruct=libsvmtrain(models(2,:)', p_pats, opts1);
    [~, l2pFam, ~]=libsvmpredict(models(2,:)', p_pats, lStruct);
    [~, p2lFam, ~]=libsvmpredict(models(2,:)', l_pats, pStruct);
    l2lFam=libsvmtrain(models(2,:)', l_pats, [opts1 ' -v 4']);
    p2pFam=libsvmtrain(models(2,:)', p_pats, [opts1 ' -v 4']);
   
    
    lStruct=libsvmtrain(models(3,:)', l_pats, opts1);
    pStruct=libsvmtrain(models(3,:)', p_pats, opts1);
    [~, l2pVerb, ~]=libsvmpredict(models(3,:)', p_pats, lStruct);
    [~, p2lVerb, ~]=libsvmpredict(models(3,:)', l_pats, pStruct);
    l2lVerb=libsvmtrain(models(3,:)', l_pats, [opts1 ' -v 4']);
    p2pVerb=libsvmtrain(models(3,:)', p_pats, [opts1 ' -v 4']);
    
    
    lStruct=libsvmtrain(models(1,:)', l_pats, opts2);
    pStruct=libsvmtrain(models(1,:)', p_pats, opts2);
    [~, l2pAnim2, ~]=libsvmpredict(models(1,:)', p_pats, lStruct);
    [~, p2lAnim2, ~]=libsvmpredict(models(1,:)', l_pats, pStruct);
    l2lAnim2=libsvmtrain(models(1,:)', l_pats, [opts2 ' -v 4']);
    p2pAnim2=libsvmtrain(models(1,:)', p_pats, [opts2 ' -v 4']);
    
   
    
    lStruct=libsvmtrain(models(2,:)', l_pats, opts2);
    pStruct=libsvmtrain(models(2,:)', p_pats, opts2);
    [~, l2pFam2, ~]=libsvmpredict(models(2,:)', p_pats, lStruct);
    [~, p2lFam2, ~]=libsvmpredict(models(2,:)', l_pats, pStruct);
    l2lFam2=libsvmtrain(models(2,:)', l_pats, [opts2 ' -v 4']);
    p2pFam2=libsvmtrain(models(2,:)', p_pats, [opts2 ' -v 4']);
   
    
    lStruct=libsvmtrain(models(3,:)', l_pats, opts2);
    pStruct=libsvmtrain(models(3,:)', p_pats, opts2);
    [~, l2pVerb2, ~]=libsvmpredict(models(3,:)', p_pats, lStruct);
    [~, p2lVerb2, ~]=libsvmpredict(models(3,:)', l_pats, pStruct);
    l2lVerb2=libsvmtrain(models(3,:)', l_pats, [opts2 ' -v 4']);
    p2pVerb2=libsvmtrain(models(3,:)', p_pats, [opts2 ' -v 4']);
    %         rs(1)=mean(l2m(1,:));
    %         rs(2)=mean(m2l(1,:));
    %     end
    %     toc
    ...
    ps=nan;
    vec_ps(cMappingVoxI,:) = ps;
    vec_rs(cMappingVoxI,:) = [l2pAnim(1) p2lAnim(1) l2lAnim(1) p2pAnim(1) ...
        l2pFam(1) p2lFam(1) l2lFam(1) p2pFam(1) ...
        l2pVerb(1) p2lVerb(1) l2lVerb(1) p2pVerb(1) ...
        l2pAnim2(1) p2lAnim2(1) l2lAnim2(1) p2pAnim2(1) ...
        l2pFam2(1) p2lFam2(1) l2lFam2(1) p2pFam2(1) ...
        l2pVerb2(1) p2lVerb2(1) l2lVerb2(1) p2pVerb2(1)];
    %%
